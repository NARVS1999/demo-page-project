// /blog/category/[slug] — published posts in one category (UI-SPEC Page 3).
// Unknown slug → 404; a category with zero published posts renders the
// empty-state (never lists drafts — RESEARCH Pitfall 4, no enumeration).
...
export default async function BlogCategoryPage({
  ...
  const categoryRows = await sql`SELECT id, name FROM categories WHERE slug = ${slug}`;
  if (categoryRows.length === 0) notFound();
  const category = categoryRows[0] as { id: string; name: string };
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex flex-col gap-3">
          ...
          <h1 className="font-serif text-4xl font-bold tracking-tight">{category.name}</h1>
          <p className="text-sm text-muted-foreground">
            {posts.length} {posts.length === 1 ? "story" : "stories"} filed under {category.name}.
          </p>
        </div>
